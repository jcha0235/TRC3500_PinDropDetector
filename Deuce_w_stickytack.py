...
def main():
    ...
    # Unique sticky tack calibration data
    cal_stickytack = {
      'Low-Near':  [[0.0526,3], [0.0520,1], [0.0531,3], [0.0515,4]],
      'Low-Far':   [[0.0541,3], [0.0569,4], [0.0512,3], [0.0522,4]],
      'High-Near': [[0.0730,12],[0.0709,19],[0.0699,16],[0.0694,15]],
      'High-Far':  [[0.064533,10],[0.06510,11],[0.06432,9], [0.06159,10]],
    }
    ...
    mat2d = {
      ...
      'stickytack': mean2d(sum(cal_stickytack.values(), [])),
    }
    ...
    hd_energy = {
      ...
      'stickytack': {hd: np.mean([e for e,_ in samples]) for hd,samples in cal_stickytack.items()},
    }
    ...
    cent2d_stickytack = {hd: mean2d([[p,e] for p,e in
                        zip([v[0] for v in cal_stickytack[hd]],[v[0] for v in cal_stickytack[hd]])])
                     for hd in cal_stickytack}
    ...
    # fallback for coin Low-Far, eraser High-Near, or potentially ambiguous stickytack cases
    if (material=='coin' and hd_bin=='Low-Far') or \
       (material=='eraser' and hd_bin=='High-Near') or \
       (material=='stickytack' and (hd_bin=='Low-Far' or hd_bin=='High-Near')):
        ...
        if material == 'coin':
            cents2d = cent2d_coin
        elif material == 'eraser':
            cents2d = cent2d_eraser
        else:  # stickytack
            cents2d = cent2d_stickytack
    ...
    plt.title(f'Centered Voltage - Classified as {material} ({hd_bin})'); 
    plt.xlabel('Time (s)'); plt.ylabel('Voltage (V)')
    plt.legend(); plt.grid(True)

    # Add a visualization of material classification
    # plt.figure(figsize=(10,6))
    # materials = ['coin', 'eraser', 'stickytack']
    # colors = ['blue', 'red', 'green']
    # markers = ['o', 's', '^']
    
    # for i, mat in enumerate(materials):
    #     data = sum([samples for samples in eval(f'cal_{mat}').values()], [])
    #     energy = [d[0] for d in data]
    #     ms_count = [d[1] for d in data]
    #     plt.scatter(energy, ms_count, c=colors[i], marker=markers[i], label=mat, alpha=0.7)
    
    # # Plot the current sample
    # plt.scatter(feats['energy_V2s'], feats['ms_count'], c='black', marker='*', s=200, 
    #             label='Current Sample')
    
    # plt.title('Material Classification')
    # plt.xlabel('Energy (V²·s)')
    # plt.ylabel('MS Count')
    # plt.legend()
    # plt.grid(True)
